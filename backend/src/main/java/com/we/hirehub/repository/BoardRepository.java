...
public interface BoardRepository extends JpaRepository<Board, Long> {
  ...
  /** 관리자 복구 시 hidden만 업데이트 (updateAt 변경 없이) */
  @Modifying(clearAutomatically = true, flushAutomatically = true)
  @Query("update Board b set b.hidden = :hidden where b.id = :boardId")
  void updateHiddenStatus(@Param("boardId") Long boardId, @Param("hidden") Boolean hidden);

  /** 관리자 승인 상태 업데이트 */
  @Modifying(clearAutomatically = true, flushAutomatically = true)
  @Query("update Board b set b.adminApproved = :approved where b.id = :boardId")
  void updateAdminApproved(@Param("boardId") Long boardId, @Param("approved") Boolean approved);
}
