...
SITE_DOCUMENT = """
[SITE NAME]
HireHub (https://noeyos.store)
한국 개발자/IT 직군 중심의 AI 기반 채용 플랫폼

-------------------------------------
📌 1. 핵심 가치
-------------------------------------
• 구직자에게는 "AI 기반 공고 추천 · 맞춤 면접준비 · 이력서 분석"
• 기업에게는 "지원자 관리 · 기업 정보 페이지 제공"
• 누구나 IT 취업을 쉽게 경험하도록 돕는 플랫폼
...
① 공고 추천 기능
- 로그인한 사용자만 이용 가능
- 사용자 정보(기술스택, 이력서, 관심 직무 등)를 기반으로 가중치 계산
- 점수가 높은 공고 순으로 추천 리스트 생성
- 비로그인 상태에서는 "모두가 주목한 공고(조회수 순)"을 보여줌

② 공고 조회
- 조회수 순, 최신순, 회사별, 카테고리별 필터 제공
- 공고 상세에서 회사 정보, 복지, 기술스택, 위치 등을 확인 가능
- 스크랩 기능 제공

③ 이력서 기능
- 이력서 등록/수정/삭제 가능
- 여러 개 등록 가능
- 지원 시 특정 이력서를 선택하여 제출
- AI 리뷰 기능으로 첨삭 가능

④ 지원 기능
- 지원하기 버튼 → 이력서 선택 → 제출 완료
- 제출 후 마이페이지에서 지원 내역 확인
- 지원 취소는 기업 정책에 따라 제한될 수 있음

⑤ 기업 정보
- 기업 이름 / 소개 / 복리후생 / 주소 / 설립년도 / 채용중인 공고 표시
- 사용자는 기업 즐겨찾기 가능
...
- 면접 질문 생성
- 면접 답변 피드백
- 스팸/비속어 필터링(Moderation)
- 키워드 기반 매칭 점수 계산

⑦ 고객센터 챗봇
- 기본 답변: AI 고객센터
- 상담사 연결 기능 존재 (WebSocket 기반)
- 10분 비활성 시 자동 연결 종료
- 상담 내역은 사용자별로 localStorage에 저장됨 (서버 DB 저장 기능 선택 가능)
...
📌 5. 정책 및 제약사항
-------------------------------------
• HireHub는 채용 중개 플랫폼이며 직접 채용을 진행하지 않음
• 공고 내용의 1차 정보는 기업 또는 외부 사이트 제공
• 개인정보는 외부에 공개되지 않으며, 상담사는 지원자 프로필을 직접 조회할 수 없음
• AI 답변은 참고용이며 법적 조언을 제공하지 않음

-------------------------------------
📌 6. 자주 발생하는 유저 질문 & 답변 규칙
-------------------------------------

① "왜 추천 공고가 안 떠요?"
→ 로그인 필요 / 가중치 기반 추천 / 데이터 부족 시 조회수 순으로 fallback
...
④ "이력서는 몇 개까지 업로드 가능해요?"
→ 제한 없음 (다만 너무 많으면 UI 상 비효율적일 수 있음)

⑤ "복지 정보는 어디서 확인해요?"
→ 기업 상세 페이지 하단

⑥ "AI 면접 질문이 자꾸 비슷해요"
→ previousQuestions 리스트를 기반으로 중복 방지함

-------------------------------------
📌 7. AI 챗봇의 답변 스타일 기준
-------------------------------------
• HireHub 운영진처럼 정확한 정보 제공
• 없는 기능은 "현재 기능에서는 제공되지 않습니다"라고 답변
• 사용자 불편 상황 → 해결 경로 제시
• 기술적 문제 → 캐시 삭제, 새로고침, 로그인 재시도 안내
• 공고/이력서 등 민감한 데이터 → "확인이 불가능합니다"로 응답
• 거짓 정보 금지, 기능 초월 금지

-------------------------------------
📌 8. 절대 답변하면 안 되는 것
-------------------------------------
• 내부 데이터 예측(지원자 수, 경쟁률 등)
• 기업의 상세 내부 정보
• 사용자 개인정보 언급
• 실제 채용 확정/합격 보장
• 사이트 서버 구조 설명
