...
def match_jobs(req: JobMatchRequest):
    """
    이력서 ID를 받아서 여러 공고와 매칭
    """
    BASE = "http://localhost:8080/api"

    try:
        # 1. Spring Boot에서 이력서 조회
        print(f"📄 이력서 조회: resumeId={req.resumeId}")
        resume_res = requests.get(f"{BASE}/mypage/resumes/{req.resumeId}", timeout=10)

        if resume_res.status_code != 200:
            print(f"❌ 이력서 조회 실패: {resume_res.status_code}")
            return {"results": []}
        ...
        # 이력서 텍스트 생성
        resume_text = f"""
이름: {resume_data.get('profile', {}).get('name', '')}
학력: {', '.join([edu.get('name', '') for edu in resume_data.get('educationList', [])])}
경력: {', '.join([career.get('companyName', '') for career in resume_data.get('careerList', [])])}
스킬: {', '.join([skill.get('name', '') for skill in resume_data.get('skillList', [])])}
자기소개: {resume_data.get('essayContent', '')}
        """.strip()[:1000]

        print(f"✅ 이력서 조회 성공")

        # 2. Spring Boot에서 공고 목록 조회
        print(f"📋 공고 목록 조회")
        jobs_res = requests.get(f"{BASE}/jobposts", timeout=10)

        if jobs_res.status_code != 200:
            print(f"❌ 공고 조회 실패: {jobs_res.status_code}")
            return {"results": []}

        jobs = jobs_res.json()
        print(f"✅ 공고 {len(jobs)}개 조회 성공")
...
요구사항: {job.get('responsibilities', '')}
우대사항: {job.get('qualifications', '')}
            """.strip()[:1000]
...
Evaluate match."""
                ...
                print(f"⚠️ 매칭 오류 (jobId={job.get('id')}): {e}")
                continue

        # 점수 순으로 정렬
        results.sort(key=lambda x: x["score"], reverse=True)

        print(f"✅ 매칭 완료: {len(results)}개 결과")
        return {"results": results}  # 최대 10개
...
class SaveMatchingRequest(BaseModel):
    resumeId: int
    resumeTitle: str
    matchResults: List[dict]

@app.post("/api/job-matching/history")
def save_matching_history(req: SaveMatchingRequest):
    """매칭 결과 저장"""
    global history_id_counter

    history_id = history_id_counter
    history_id_counter += 1
    ...
    history_data = {
        ...
        "matchResults": req.matchResults,
        "createdAt": datetime.now().isoformat()
    }

    matching_history[history_id] = history_data
    print(f"💾 매칭 이력 저장: id={history_id}, resumeId={req.resumeId}")

    return history_data

@app.get("/api/job-matching/history")
def get_matching_history():
    """매칭 이력 목록 조회"""
    history_list = list(matching_history.values())
    history_list.sort(key=lambda x: x["createdAt"], reverse=True)
    print(f"📋 매칭 이력 조회: {len(history_list)}개")
    return history_list

@app.get("/api/job-matching/history/{history_id}")
def get_matching_history_detail(history_id: int):
    """매칭 이력 상세 조회"""
    if history_id in matching_history:
        return matching_history[history_id]
    return {"error": "History not found"}, 404

@app.delete("/api/job-matching/history/{history_id}")
def delete_matching_history(history_id: int):
    """매칭 이력 삭제"""
    if history_id in matching_history:
        del matching_history[history_id]
        print(f"🗑️ 매칭 이력 삭제: id={history_id}")
        return {"success": True}
    return {"error": "History not found"}, 404
