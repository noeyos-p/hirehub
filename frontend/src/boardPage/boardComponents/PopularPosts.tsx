        ...
        {popularBoards.length === 0 ? (
          ...
          <div className="space-y-3 md:space-y-4 mb-0">
            {popularBoards.map((board) => (
              <div
                key={board.id}
                className="border-b border-gray-200 pb-3 md:pb-4 last:border-b-0 cursor-pointer hover:bg-gray-100 transition p-1.5 md:p-2 rounded"
                onClick={() => handleBoardClick(board.id)}
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center space-x-2 md:space-x-3 min-w-0 flex-1">
                    <div className="w-8 h-8 md:w-10 md:h-10 bg-gray-300 rounded-full flex items-center justify-center overflow-hidden flex-shrink-0">
